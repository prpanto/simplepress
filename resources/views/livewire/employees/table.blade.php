        ...
        <table class="w-full mt-6 w-full whitespace-nowrap text-left">
            <thead class="border-b border-white/10 text-sm leading-6 text-white">
                <tr>
                    ...
                    <th scope="col" class="py-2 pl-4 pr-8 font-semibold sm:pl-6 lg:pl-8">Fullname</th>
                    <th scope="col" class="py-2 pl-4 pr-8 font-semibold">Email</th>
                    <th scope="col" class="py-2 pl-4 pr-4 text-right font-semibold sm:pr-8 sm:text-left lg:pr-20">Phone</th>
                ...
                @foreach ($employees as $employee)
                    <tr>
                        <td class="py-4 pl-4 pr-4 sm:pl-6 lg:pl-8">
                            <span class="truncate text-sm font-medium leading-6 text-gray-200">{{ $employee->id }}</span>
                        </td>
                        <td class="py-4 pl-4 pr-4 sm:pl-6 lg:pl-8">
                            <span class="truncate text-sm font-medium leading-6 text-gray-200">{{ $employee->fullname }}</span>
                        </td>
                        <td class="py-4 pl-4 pr-4 sm:pr-8">
                            <span class="text-sm text-gray-400">{{ $employee->email }}</span>
                        </td>
                        <td class="py-4 pl-4 pr-4 text-sm leading-6 sm:pr-8 lg:pr-20">
                            <span class="text-gray-400 line-clamp-1 whitespace-normal">{{ $employee->phone }}</span>
                        </td>
                        <td class="py-4 pl-4 pr-4 text-right text-sm leading-6 text-gray-400 sm:pr-6 lg:pr-8">
                            <time datetime="{{ $employee->created_at }}">{{ $employee->created_at->diffForHumans() }}</time>
                        </td>
                        <td class="py-4 pl-4 sm:pr-8 text-sm">
                            <div class="flex items-center space-x-2">
                                @if (auth()->user()->can('view:employees'))
                                    <livewire:employees.table-view :id="$employee->id" />
                                @endif
                                @if (auth()->user()->can('edit:employees'))
                                    <a href="{{ route('employees.edit', $employee->id) }}" class="text-gray-900 text-sm font-semibold bg-yellow-400 px-2.5 py-1 rounded-md flex items-center space-x-1">
                                ...
                                @if (auth()->user()->can('delete:employees'))
                                    <livewire:employees.table-delete :id="$employee->id" />
                                @endif
                            </div>
                        </td>
                    </tr>
